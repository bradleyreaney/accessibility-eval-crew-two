...
class Phase2Demo:
    ...
    def _load_data(self):
        ...
        for plan_file in plan_files:
            try:
                ...
                self.plan_contents[plan_file.stem] = (
                    f"Sample content for {plan_file.stem} - accessibility remediation plan with comprehensive approach."
                )

        logger.info(
            f"Loaded {len(self.plan_contents)} remediation plans for evaluation"
        )
    ...
    def run_strategic_analysis(self) -> Dict[str, Any]:
        ...
        executive_summary = self.agents["analysis_agent"].generate_executive_summary(
            all_data
        )
    ...
    def _print_scoring_summary(self, scoring_results: Dict[str, Any]):
        ...
        print(
            f"Evaluations Processed: {scoring_results.get('evaluations_processed', 0)}"
        )
    ...
    def _print_strategic_summary(self, strategic_analysis: Dict[str, Any]):
        ...
        recommendation = strategic_analysis.get(
            "primary_recommendation", "See full analysis"
        )
    ...
    def _print_demo_summary(self):
        ...
        print(
            f"✅ Scoring Analysis: {'Completed' if self.scoring_results.get('success') else 'Failed'}"
        )
        print(
            f"✅ Strategic Analysis: {'Completed' if self.strategic_analysis.get('success') else 'Failed'}"
        )
    ...
    def test_individual_components(self):
        ...
        if hasattr(eval_tool, "criteria_weights"):
            logger.info(
                f"✅ Evaluation framework loaded with {len(eval_tool.criteria_weights)} criteria"
            )
