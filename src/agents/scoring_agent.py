...
class ScoringAgent:
    ...
    def calculate_final_scores(self, evaluations: List[Dict[str, Any]], 
                             criteria_weights: Dict[str, float]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for plan_name, scores in plan_scores.items():
                if scores:  # Only process plans with valid scores
                    weighted_score = self._calculate_weighted_score(scores, criteria_weights)
    ...
    def compare_plans(self, plan_a: Dict[str, Any], plan_b: Dict[str, Any],
        ...
        try:
            ...
            logger.info(f"Plan comparison completed: {plan_a['name']} vs {plan_b['name']}")
    ...
    def _extract_scores_from_evaluations(self, evaluations: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
    ...
    def _generate_rankings(self, weighted_scores: Dict[str, float]) -> List[Tuple[str, float]]:
        """Generate ranked list of plans by score"""
        return sorted(weighted_scores.items(), key=lambda x: x[1], reverse=True)
    
    def _format_evaluations_for_analysis(self, evaluations: List[Dict[str, Any]]) -> str:
