...
class EvaluationFrameworkTool(BaseTool):
    ...
    def __init__(self):
        ...
        prompt_path = (
            Path(__file__).parent.parent.parent.parent / "promt" / "eval-prompt.md"
        )
    ...
    def _load_evaluation_framework(self) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(
                f"Loaded evaluation framework with {len(criteria_weights)} criteria"
            )
    ...
    def _run(self, plan_name: str, plan_content: str, audit_context: str) -> str:
        ...
        try:
            ...
            return (
                f"Error: Failed to apply evaluation framework to {plan_name}: {str(e)}"
            )
