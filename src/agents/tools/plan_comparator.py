...
class PlanComparatorTool(BaseTool):
    ...
    def __init__(self):
        ...
        # Comparison dimensions for analysis
        self.comparison_dimensions = {
            "scope": ["comprehensive", "scope", "coverage", "breadth", "wide", "narrow"],
            "depth": ["detailed", "specific", "depth", "thorough", "surface", "deep"],
            ...
            "methodology": ["approach", "method", "strategy", "technique", "framework"],
            "testing": ["test", "validate", "verify", "check", "audit"],
            "maintenance": ["maintain", "monitor", "ongoing", "continuous", "sustain"]
        }
    
    def _run(self, plan_a_name: str, plan_a_content: str, plan_b_name: str, 
             plan_b_content: str, comparison_criteria: List[str]) -> str:
        """
        ...
        try:
            ...
            comparison_report = self._generate_comparison_report(
                plan_a_name, plan_b_name, plan_a_analysis, plan_b_analysis,
                dimensional_comparison, unique_strengths
            )
    ...
    def _analyze_plan_characteristics(self, plan_content: str) -> Dict[str, Any]:
        ...
        # Technical depth
        technical_words = ["implement", "code", "development", "technical", "specific", "detailed"]
        characteristics["technical_depth"] = sum(
            content_lower.count(word) for word in technical_words
        )
        
        # Strategic elements
        strategic_words = ["strategy", "approach", "methodology", "framework", "philosophy"]
        characteristics["strategic_elements"] = sum(
            content_lower.count(word) for word in strategic_words
        )
        
        # Timeline mentions
        timeline_words = ["timeline", "schedule", "deadline", "milestone", "phase", "week", "month"]
    ...
    def _compare_dimensions(self, plan_a: str, plan_b: str, name_a: str, name_b: str) -> Dict[str, Dict[str, Any]]:
    ...
    def _identify_unique_strengths(self, plan_a: str, plan_b: str, name_a: str, name_b: str) -> Dict[str, List[str]]:
        """Identify unique strengths of each plan"""
        strengths = {name_a: [], name_b: []}
        
        # Define strength indicators
        strength_patterns = {
            "Comprehensive Coverage": ["comprehensive", "complete", "thorough", "extensive"],
    ...
    def _generate_comparison_report(self, name_a: str, name_b: str, 
                                  analysis_a: Dict[str, Any], analysis_b: Dict[str, Any],
                                  dimensional_comparison: Dict[str, Dict[str, Any]],
                                  unique_strengths: Dict[str, List[str]]) -> str:
        """Generate comprehensive comparison report"""
        ...
        for dimension, data in dimensional_comparison.items():
            ...
            if advantage != "Equal":
                report_lines.append(f"  {dimension.title()}: {advantage} (+{difference})")
            else:
    ...
    def _generate_strategic_recommendations(self, name_a: str, name_b: str,
                                          analysis_a: Dict[str, Any], analysis_b: Dict[str, Any],
                                          dimensional_comparison: Dict[str, Dict[str, Any]]) -> List[str]:
        """Generate strategic recommendations based on comparison"""
        recommendations = []
        
        # Length-based recommendations
        if analysis_a['length'] > analysis_b['length'] * 1.5:
            recommendations.append(f"{name_a} provides more comprehensive coverage but may be resource-intensive")
        elif analysis_b['length'] > analysis_a['length'] * 1.5:
            recommendations.append(f"{name_b} provides more comprehensive coverage but may be resource-intensive")
        
        # Technical depth recommendations
        if analysis_a['technical_depth'] > analysis_b['technical_depth'] * 2:
            recommendations.append(f"{name_a} offers superior technical specificity for complex implementations")
        elif analysis_b['technical_depth'] > analysis_a['technical_depth'] * 2:
            recommendations.append(f"{name_b} offers superior technical specificity for complex implementations")
        
        # Timeline recommendations
        if analysis_a['timeline_mentions'] > analysis_b['timeline_mentions'] * 2:
            recommendations.append(f"{name_a} provides clearer project timeline and milestone planning")
        elif analysis_b['timeline_mentions'] > analysis_a['timeline_mentions'] * 2:
            recommendations.append(f"{name_b} provides clearer project timeline and milestone planning")
        
        # Testing recommendations
        if analysis_a['testing_focus'] > analysis_b['testing_focus'] * 2:
            recommendations.append(f"{name_a} emphasizes quality assurance and validation processes")
        elif analysis_b['testing_focus'] > analysis_a['testing_focus'] * 2:
            recommendations.append(f"{name_b} emphasizes quality assurance and validation processes")
