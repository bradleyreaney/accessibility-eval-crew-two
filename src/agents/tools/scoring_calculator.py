...
class ScoringInput(BaseModel):
    ...
    criterion_scores: Dict[str, float] = Field(description="Dict of criterion name to score (1-10)")
    criteria_weights: Dict[str, float] = Field(description="Dict of criterion name to weight (0-1)")
...
class ScoringCalculatorTool(BaseTool):
    ...
    def _run(self, criterion_scores: Dict[str, float], criteria_weights: Dict[str, float], 
             plan_name: str) -> str:
        """
        ...
        try:
            ...
            weighted_score = self._calculate_weighted_score(criterion_scores, criteria_weights)
    ...
    def _calculate_weighted_score(self, scores: Dict[str, float], weights: Dict[str, float]) -> float:
        ...
        if total_weight > 0:
            return total_weighted_score / total_weight if total_weight != 1.0 else total_weighted_score
        return 0.0
    
    def _generate_scoring_analysis(self, scores: Dict[str, float], weights: Dict[str, float], 
                                 weighted_score: float, plan_name: str) -> str:
